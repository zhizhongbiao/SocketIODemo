package cn.com.tianyudg.socketiodemo.util.encrypt;
...
public class AESEncrypt {
    ...
    private static String ivParameter = SocketIoConfig.ENCRYPT_STRINRG.IVPARAMETER;// 密钥默认偏移，可更改
    // private byte[] iv = { 0xA, 1, 0xB, 5, 4, 0xF, 7, 9, 0x17, 3, 1, 6, 8,
    // 0xC,
    // 0xD, 91 };
    private byte[] iv = ivParameter.getBytes();
    private IvParameterSpec IV;
    String sKey = SocketIoConfig.ENCRYPT_STRINRG.SKEY;// key必须为16位，可更改为自己的key
    ...
    private byte[] encrypt(String cmp, SecretKey sk, IvParameterSpec IV, byte[] msg) {
        try {
            Log.e(TAG, "encrypt: ++cmp=="+cmp);
            Log.e(TAG, "encrypt: ++sk=="+sk);
            Log.e(TAG, "encrypt: ++IV=="+IV);
            Log.e(TAG, "encrypt: ++msg=="+msg.toString());
